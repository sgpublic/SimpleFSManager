...
export function App() {
  ...
  if (status.isLoading) return <AuthShell><p className="text-sm text-slate-400">{t("auth.checking")}</p></AuthShell>;
  if (status.isError) return <AuthShell><p className="text-sm text-rose-300">{t("auth.unavailable")}</p></AuthShell>;
  if (status.data?.setupRequired) return <Bootstrap onComplete={() => void status.refetch()} />;
  if (!status.data?.authenticated) return <Login username={status.data?.username ?? ""} onComplete={() => void status.refetch()} />;
  return <Dashboard username={status.data.username ?? ""} onLogout={() => void status.refetch()} />;
}
...
function AuthShell({ children }: { children: ReactNode }) { return <main className="relative grid min-h-screen place-items-center bg-slate-950 px-6 text-slate-100"><div className="absolute right-6 top-6"><LanguageSelect /></div><div className="w-full max-w-md">{children}</div></main>; }
