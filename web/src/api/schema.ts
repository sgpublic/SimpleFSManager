...
export interface paths {
  ...
  "/api/disks": { get: { responses: { 200: { content: { "application/json": { disks: Array<{ path: string; name: string; model: string; serial: string; sizeBytes: number; partitioning: string; transport: string; usb: boolean; protected: boolean; system: boolean; reclaimable: boolean; temperatureCelsius?: number; smartHealth?: boolean; mountpoints: string[]; partitions: Array<{ path: string; name: string; number: number; sizeBytes: number; fileSystem: string; uuid: string; mountpoints: string[]; usage?: { totalBytes: number; usedBytes: number; availableBytes: number } }> }> } } } } } };
}
